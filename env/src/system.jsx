...
export class Menu extends React.Component {
    ...
    componentDidMount(){
        i18n.load_file().then(()=>{
            ...
            alert("ERROR: CANNOT LOAD LOCALIZATION FILE.");
        });
    }
...
class Title extends React.Component{
    ...
    render(){
        ...
        <div id="title-div" >
            ...
            <div id="version">Player Version: beta-0.2.0<br/>Sonolus Version: beta-0.8.4</div>
        </div>);
    }
}
...
const TOS = props => {
    return <div id="rule">
        ...
        1. 利用者がこのウェブシステム(*1)で、管理者の意図していない行為(*2)をすることを禁止する。<br/>
        2. このウェブシステムにより発生した直接損害・間接損害の責任の一切を管理者は負わないものとする。<br/>
        3. Sonolus custom servers(以下「外部サーバー」と呼称する)から提供されるサーバー内容・サーバー概要や、楽曲内容・楽曲概要、譜面内容・譜面概要、そのほかのソースデータ(*3)・その概要とは管理者は一切関与せず、利用者の自己責任としてデータが取得される。<br/>
        4. このウェブシステムは、フリーズ、クラッシュ、意図しない挙動、または(「および」の意も含む。以下同様)これらによる利用者やこのウェブシステムが実行されているデバイスへの影響が発生する可能性があり、すべて利用者の自己責任となる。<br/>
        5. このウェブシステムが利用しているこのウェブシステム以外のサービス(以下「外部サービス」と呼称する)が、このウェブシステムによる通信内容を記録、解析、利用、そのほかの行為をすることを利用者は承諾する。<br/>
        6. 管理者は、ある特定の利用者に対する管理者の明確かつ具体的な許可により、上記の禁止されている行為のうち管理者によって許可された行為のみ、特定の利用者が履行することを許す。<br/>
        7. 管理者はこのウェブシステム上で管理者の在住している国の法律および管理者の在住している国が批准している国際法の範囲内ですべての行為を実行できる。<br/>
        <br/>
        *1「このウェブシステム」とは、https://1217pond.github.io/sonolus-web-player/ 以下の利用者がアクセス可能なファイルによって構築された、HTMLレンダリングエンジン上で実行されるシステムのこと。<br/>
        *2 以下の行為を指す。<br/>
        ...
        b. 管理者の意図した操作以外を利用して、故意に、このウェブシステムが実行する処理、通信、その他の動作が、阻害、停止・中止、そのほかの管理者の意図しない挙動をするように操作する行為(例：コンソールの操作によりインターネット通信を改竄する行為)<br/>
        c. 管理者の意図した操作を利用しているかどうかに関係なく、故意に、管理者が正常ではないと判断した、または判断する動作を実行する行為(例：管理者がバグが発生すると判断した行為を故意に行う行為)<br/>
        d. 管理者の意図した操作を利用しているかどうかに関係なく、故意であるかどうかに関係なく、外部サービス、外部サーバーに対する、必要以上の負荷を与える行為、損害を与える行為、そのほかの悪意をもった行為(例：外部サーバーに通信を行うボタンを必要以上に押下し、外部サーバーに必要以上の負荷を与える行為)<br/>
        *3 Sonolusで利用される、スキン、背景、エフェクト、エンジン、パーティクルのデータ<br/>
        </div>
        <a onClick={()=>{
            props.C.back_menu();
...
class Server_Menu extends React.Component{
    ...
    async componentDidMount(){
        ...
        console.log("hoge", servers, collections);
        this.setState({
            Server_list: servers,
    ...
    return(
        <div className="menu-selector" onClick={async()=>{
            if(C.menu_route[C.menu_route.length-1] != "EditServerMenu"){
                ...
                request_server_info(ADDRESS).then(result=>{
                    TEMP.server_info_data = result;
                    console.log(result);
...
async function request_server_info(address){
    ...
    //アイテムの互換性
    // if(response.levels.items){
    //     response.levels = response.levels.items;
    // }
    // if(response.skins.items){
    //     response.skins = response.skins.items;
    // }
    // if(response.backgrounds.items){
    //     response.backgrounds = response.backgrounds.items;
    // }
    // if(response.effects.items){
    //     response.effects = response.effects.items;
    // }
    // if(response.particles.items){
    //     response.particles = response.particles.items;
    // }
    // if(response.engines.items){
    //     response.engines = response.engines.items;
    // }
    // return response;
}
...
class Img_custom extends React.Component {
    constructor(props){
        ...
        if(this.src in THUMB_TMP){
            ...
            //     });
            //     this.requestable = true;
            //     console.log(`thumbnail res: ERROR`,error);
            // });
        }
...
class Item_List_Info extends React.Component {
    ...
    open_info(){  
        if(!this.data_loaded){
            $("#loading-cover").show();
            request(ADDRESS,`/sonolus/${this.item_type}/info`,{localization:LOCALIZATION}).then(result=>{
                this.info_data = JSON.parse(result);
...
async function request_server_more(address,type,page_index,querys = {}){//thisはMenuControllerになってる
    console.log(querys);
...
function request(domain,endpoint,params){
...
class Post_Selector extends React.Component{
    constructor(props){
        ...
        this.thumbnail = post.thumbnail.url||"./textures/unknown.png";
...
class PlayList_Selector extends React.Component{
    constructor(props){
        ...
        this.thumbnail = (playlist.thumbnail||{url:"./textures/unknown.png"}).url;
...
class Level_Selector extends React.Component{
    constructor(props){
        ...
        this.thumbnail = level.cover.url||"./textures/unknown.png";
...
class Item_Selector extends React.Component{
    constructor(props){
        ...
        this.thumbnail = (item.thumbnail||{url:"./textures/unknown.png"}).url;
...
async function request_item_info(address,type,name){
    let response = await request(address, `/sonolus/${type}/${name}`, {
        localization:LOCALIZATION
    });
...
class Server_More extends React.Component{
        ...
        return(
            <div id="serv-more">
                ...
                <div className="selector-window" id="filter">
                    ...
                    <button id="search" className="btn" onClick={async ()=>{
                        ...
                        if(correct){
                            ...
                            TEMP.server_more_data = await request_server_more(ADDRESS,TEMP.item_type,this.state.page_index,queries);
                            console.log(TEMP.server_more_data);
                            this.setState({
                            ...
                            alert("⚠️マークのある検索項目が初期値のままでは検索できません。");
                        }
                    ...
                    <button id="cancel" className="btn red-btn" onClick={async()=>{
                        this.queries = {};
                        $("#loading-cover").show();
                        TEMP.server_more_data = await request_server_more(ADDRESS,TEMP.item_type,this.state.page_index);
                        console.log(TEMP.server_more_data);
                        this.setState({
                            search_type:this.state.search_type,
                            page_index:this.state.page_index
                ...
                <div className="selector-window" id="window">
                    ...
                    <p id="page-number">{this.state.page_index+1}/{TEMP.server_more_data.pageCount}</p>
                </div>
            </div>
        );
    }
    ...
    next_button(){
        ...
        return (
            <button 
                id="next" 
                disabled={this.state.page_index+1 >= TEMP.server_more_data.pageCount}
                className="btn"
                onClick={async()=>{
                    $("#loading-cover").show();
                    let _page_index = this.state.page_index+1;
...
class Search_Option extends React.Component{
    ...
    render(){
        ...
        switch(this.option.type){
            ...
            case "select":
                return <div>
                    <span>{l(this.option.name)}</span>&emsp;<select defaultValue={this.search_queries[this.option.query]} onChange={(e)=>{this.search_queries[this.option.query] = e.target.value;}}>
        ...
        return(
            <div id="level-info">
                ...
                <div className="selector-window" id="info" style={{display:"flex"}}>
                    <figure className="level-cover"><Img_custom src={join_path(ADDRESS,item.cover.url)} style={{width:"300px",height:"300px"}}></Img_custom></figure>
                        ...
                        <button id="add-coll" className="btn" onClick={async()=>{
                            ...
                            if(collection_name){
                                ...
                                if(coll_item_names.includes(this.state.data.item.name+"(level)")){
                                    ...
                                    add_collection_item(ADDRESS,collection_name,this.state.data.item,"levels");
                                    alert(t("追加完了"));
                                }
                                $("#loading-cover").hide();
                            }else{
...
const Item_Selector_on_LevelInfo = props => {
    ...
    if(props.item.unselected){
        ...
        if("type" in item){
            // thumbnail = item.thumbnail||"./textures/unknown.png";
            thumbnail = URL.createObjectURL(item.thumbnail);
        }else{
            thumbnail = (item.thumbnail||{url:"./textures/unknown.png"}).url;
        }
...
class Ref_Selector_on_LevelInfo extends React.Component{
    constructor(props){
        ...
        if(props.item.default){
            this.thumbnail = (this.item.thumbnail||{url:"./textures/unknown.png"}).url;
            this.title = this.item.title;
        }else{
            // this.thumbnail = this.item.thumbnail||"./textures/unknown.png";
...
async function get_collection_items(collection_name){
    ...
    // if(collection.version != 2){//base64対策
    //     collection.version = 2;
    //     for(let [name,item] of Object.entries(collection.data)){
    //         switch(item.type){
    //             case "backgrounds":
    //                 if(typeof item.background_img == "string"){
    //                     collection.data[name].background_img = Uint8ArrayToblob(base64ToUint8Array(item.background_img));
    //                 }
    //                 break;
    //             case "effects":
    //                 for(let [audio_name,effect] of Object.entries(item.effect_audio)){
    //                     if(typeof effect == "string"){
    //                         collection.data[name].effect_audio[audio_name] = Uint8ArrayToblob(base64ToUint8Array(effect));
    //                     }
    //                 }
    //                 break;
    //             case "particles":
    //                 if(typeof item.particle_tex == "string"){
    //                     collection.data[name].particle_tex = Uint8ArrayToblob(base64ToUint8Array(item.particle_tex));
    //                 }
    //                 break;
    //             case "skins":
    //                 if(typeof item.texture_img == "string"){
    //                     collection.data[name].texture_img = Uint8ArrayToblob(base64ToUint8Array(item.texture_img));
    //                 }
    //                 break;
    //         }
    //     }
                        ...
                        <button id="next" className="btn" onClick={async()=>{
                            ...
                            if(collection_name){
                                ...
                                if(coll_item_names.includes(`${this.state.data.item.name}(${plural2singular(this.type)})`)){
                                    ...
                                    add_collection_item(ADDRESS,collection_name,this.state.data.item,this.type);
                                    alert(t("追加完了"));
                                }
                                $("#loading-cover").hide();
                            }else{
...
function add_collection_item(address,collection_name,item,type){
    console.log(item);
    db.collections.get(collection_name).then(async e => {
        if(type == "levels"){
...
async function get_content(domain,endpoint,hash){
    if(hash){
        ...
        if(file){
            ...
            //     file.file = Uint8ArrayToblob(base64ToUint8Array(file.file));
            //     db.cache.put(file);
            // }
            return file.file;
        }else{
...
const LOCALIZATION = URL_query.has("lang")?URL_query.get("lang"):window.navigator.language;
console.log("language:",LOCALIZATION);
